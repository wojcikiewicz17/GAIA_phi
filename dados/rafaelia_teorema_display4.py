...
def print_centered(text: str, color: str = "white"):
    ...
    Se a linha for maior que a largura do terminal, imprime sem centralizar
    para evitar “quebrar” palavras embaixo.
    """
    ...
    for line in text.splitlines():
        ...
        if len(stripped) >= cols:
            # Sem padding: evita quebrar visualmente
...
def animate_triangle(a: float, b: float):
    ...
    lines = [
        ("Teorema de Rafael", "yellow"),
        ("", "white"),
        ("  c² = 2ab + (a - b)²", "white"),
        (f"  para a = {a:.3f}, b = {b:.3f}:", "white"),
        (f"  c² = {2*a*b:.3f} + {(a-b)**2:.3f} = {c2:.3f}", "white"),
        (f"  c  = √{c2:.3f} ≈ {c_len:.3f}", "white"),
        ("", "white"),
        ("Constante Rafaeliana", "yellow"),
        (f"  φ_R = √3 / 2 ≈ {phiR:.6f}", "white"),
        ("", "white"),
        ("Leituras geométricas:", "magenta"),
        ("  • Altura do triângulo equilátero unitário", "white"),
        ("  • Meia-diagonal normalizada do cubo unitário", "white"),
        ("  • Fator de inclinação natural 3–6–9", "white"),
    ]

    for i, (line, colr) in enumerate(lines):
        sleep(0.12 + i * 0.01)
        print(c(line, colr))
...
def show_full_composition(a: float, b: float):
    """Anima blocos e finaliza com tela estática completa."""
    ...
    # Texto matemático à esquerda, sem centralizar
    c2 = teorema_rafael(a, b)
    c_len = math.sqrt(c2)
    phiR = constante_rafaeliana()
...
def main():
    # Usa os valores “clássicos” que você mostrou nas telas
    a = 3000.0
    b = 2000.0
